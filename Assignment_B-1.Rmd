...
Try to find the average number of vacancy rooms of the Hotel_Dataset for each city by passing the
dataset, City, and Vacancy_rooms respectively into my function. 
```{R}
mean_of_each_group(Hotel_Dataset, "City", "Vacancy_rooms")
```
 
Try to find the average price of the Hotel_Dataset for each city by passing the
dataset, City, and Price respectively into my function. 
```{R}
...
Try to test whether it works as expected in normal case. And in the following code, I compare the averaged vacant rooms and their corresponding cities. 
```{r}
